...
with open(dataFile, 'rb') as csvfile:
   ...
   for row in reader:
      ...
      errList.append(float(row['errors']))

MinX = 0
MaxX = 55
MinY = 0.9*min(errList)
MaxY = 1.1*max(errList)

#p = plt.plot(nList,errList, 'k-', label='label')
p = plt.plot(nList,errList, 'k-', label=None)
...
plt.ylabel('Average error', fontsize=22)
